<template>
  <div>
    <div
      v-if="(MemberSideBarStatus || UserBarStatus) && Width < BreakPoint.md"
      ...
      <b-navbar-nav class="ml-auto">
        <div class="navbarCtrl">
          ...
          >
            ...
            <b-nav-item v-else to="/login"> {{$t("Header.SingIn")}} </b-nav-item>
          </b-row>
          <Header-Ctrl />
        </div>
      </b-navbar-nav>
    ...
    <b-navbar class="header mobile-navbar-display">
      <div style="color: #c4c6c8">
        <icon
          :icon="MemberSideBarStatus ? SideIcon[0] : SideIcon[1]"
    ...
    <MemberSidebar
      :style="{ 'margin-left': MemberSideBarStatus ? '0px' : ' -200px ' }"
    />
    <Userbar
      ref="UserBar"
      id="UserBar"
...
export default {
  ...
  methods: {
    ...mapMutations("Render", [
      "UpdateMemberSideBarStatus",
      "UpdateUserBarStatus",
    ]),
    openSideBar() {
      this.UpdateMemberSideBarStatus("");
    ...
    openUserBar() {
      this.UpdateUserBarStatus("");
      if (this.MemberSideBarStatus) {
        this.UpdateMemberSideBarStatus("");
      }
    },
    backDrop() {
      this.BackDropStatus = false;
      this.UpdateMemberSideBarStatus(false);
      this.UpdateUserBarStatus(false);
    },
    widthCtrl() {
      if (this.Width >= this.BreakPoint.md) {
        this.UpdateMemberSideBarStatus(true);
        this.UpdateUserBarStatus(false);
      } else {
        this.UpdateMemberSideBarStatus(false);
      }
